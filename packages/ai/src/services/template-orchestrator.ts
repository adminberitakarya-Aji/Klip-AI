...
class FFmpegService {
  ...
  private constructor() {
    // Use system ffmpeg or bundled binary
  ...
  async concatVideos(
    ...
    try {
      ...
      if (options.codec === "copy") {
        // Fast, lossless concat (requires same codec/resolution/fps)
        args.push("-c", "copy");
      } else {
  ...
  async concatWithTransitions(
    ...
    const { stdout, stderr } = await execFileAsync(this.ffmpegPath, args, {
      timeout: 600000, // 10 minutes for transitions
    });
