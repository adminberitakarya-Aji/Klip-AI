...
import { Mesh, Group, SphereGeometry, MeshStandardMaterial, Vector3 } from 'three';
...
export function AuroraOrbs({ count = 5, radius = 8, speed = 1 }: AuroraOrbsProps) {
  const groupRef = useRef<Group>(null);
  const orbs = useMemo(() => {
    const arr: { ref: any; phase: number; speed: number; axis: Vector3; color: string }[] = [];
  ...
  useFrame(() => {
    ...
    orbs.forEach((orb, i) => {
      ...
      if (orb.ref.current) {
        ...
        orb.ref.current.scale.setScalar(0.8 + Math.sin(t * 2 + orb.phase) * 0.2);
      }
    });
  });
